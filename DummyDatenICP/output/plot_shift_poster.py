...
def main():
    ...
    with H5PointCloudStream(h5_path) as stream:
        # 2. Referenz Frame (0) laden und säubern
        print("Lade Referenz (Frame 0)...")
        pcd_ref = stream.get_pcd(0, roi_bbox=roi_bbox, voxel_size=1.5)
        pcd_ref = extract_main_cluster(pcd_ref)
        pcd_ref.paint_uniform_color([0.1, 0.5, 0.8])  # Poster-Blau
        pcd_ref.estimate_normals()  # Für schöne Schattierung im Viewer

        # 3. Verschobenen Frame laden und säubern
        print(f"Lade verschobenes Phantom (Frame {SHIFT_FRAME_INDEX})...")
        pcd_shift = stream.get_pcd(SHIFT_FRAME_INDEX, roi_bbox=roi_bbox, voxel_size=1.5)
        pcd_shift = extract_main_cluster(pcd_shift)
        pcd_shift.paint_uniform_color([0.9, 0.4, 0.1])  # Poster-Orange
        pcd_shift.estimate_normals()  # Für schöne Schattierung

    # 4. Visualisierung einstellen
    print("\n--- Viewer für Poster-Screenshot geöffnet ---")
    print("Drücke 'H' im 3D-Fenster für Licht-Schatten-Effekte.")
    print("Mit 'P' kannst du das Bild direkt speichern.")
